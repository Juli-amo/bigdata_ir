...
class FeatureIndexer:
    ...
    def _extract_comprehensive_features(
        self, image_database: ImageDatabase, image_id: str
    ) -> Optional[np.ndarray]:
        """
        ...
        try:
            ...
            with sqlite3.connect(image_database.db_path) as conn:
                cursor = conn.cursor()
                cursor.execute(
                    "SELECT texture_features FROM advanced_features WHERE image_id = ?",
                    (image_id,),
                )
                row = cursor.fetchone()
...
class ApproximateNearestNeighbor:
    ...
    def build_index(self, feature_matrix: np.ndarray, image_ids: list[str]):
        ...
        self.logger.info(
            f"{self.algorithm} Index gebaut in {build_time:.2f}s für {len(image_ids)} Bilder"
        )
        self.is_built = True

    def _build_brute_force_index(self):
        """Brute Force - einfach Matrix speichern"""
        self.index = {"type": "brute_force", "matrix": self.feature_matrix, "ids": self.image_ids}
    ...
    def _build_lsh_index(self):
        ...
        _n_bits = self.kwargs.get("n_bits", 8)
    ...
    def _build_tree_index(self):
        ...
        nn_index = NearestNeighbors(algorithm=algorithm, leaf_size=leaf_size, metric="cosine")
        nn_index.fit(self.feature_matrix)

        self.index = {"type": "tree_based", "nn_index": nn_index, "ids": self.image_ids}
    ...
    def _build_faiss_index(self):
        ...
        try:
            ...
            self.index = {"type": "faiss", "faiss_index": index, "ids": self.image_ids}
...
class SearchEngine:
    ...
    def build_search_index(
        self, force_rebuild: bool = False, use_pca: bool = None, pca_components: int = 50
    ):
        """
        Baut Suchindex auf
    ...
    def search_similar_images(
        self, query_image_path: str, k: int = 5, similarity_threshold: float = 0.0
    ) -> list[dict]:
        """
        ...
        try:
            ...
            query_features = self._extract_query_features(
                query_image, color_analyzer, feature_extractor
            )
            ...
            self.logger.info(
                f"Suche abgeschlossen in {search_time:.3f}s - {len(enriched_results)} Ergebnisse"
            )
    ...
    def _extract_query_features(self, image, color_analyzer, feature_extractor) -> np.ndarray:
        ...
        # 6. Color Diversity
        color_diversity = (
            np.std(dominant_colors.reshape(-1, 3), axis=0)
            if len(dominant_colors) > 3
            else [0, 0, 0]
        )
    ...
    def benchmark_search_performance(
        self, test_image_path: str, k_values: list[int] = [1, 5, 10, 20]
    ) -> dict:
        """
        ...
        Returns: dict: Performance-Statistiken
        """
        results = {}
...
def create_search_engine(
    image_directory: str, ann_algorithm: str = "auto", db_path: str = "image_recommender.db"
) -> SearchEngine:
    """
